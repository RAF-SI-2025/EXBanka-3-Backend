...
// EmailSender is the minimal interface used by the cron price-alert check
// and by tests (via a mock implementation).
type EmailSender interface {
	Send(to, subject, body string) error
}

// SMTPEmailService sends plain-text emails via a Mailhog-compatible SMTP
// server (no authentication required, matching the pattern used by
// account-service and auth-service).
